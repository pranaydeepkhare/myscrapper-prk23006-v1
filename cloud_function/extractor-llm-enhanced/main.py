# main.py - ENHANCED LLM EXTRACTOR
# Purpose: Extract EXTENDED fields including color, city, state, zip_code
# Location: cloud_function/extractor-llm-enhanced/main.py
...
def _vertex_extract_fields(raw_text: str) -> dict:
    ...
    # ENHANCED JSON schema with 4 NEW fields
    schema = {
        "type": "object",
        "properties": {
            # Original fields
...
# -------------------- HTTP ENTRY --------------------
def llm_extract_enhanced_http(request: Request):
    """
    ENHANCED LLM Extractor with 10 fields (6 original + 4 new).
    Reads latest (or requested) run's per-listing JSONL inputs and writes enhanced LLM outputs.
    """
    ...
    if LLM_PROVIDER != "vertex":
        return jsonify({"ok": False, "error": "Enhanced extractor supports LLM_PROVIDER='vertex' only"}), 400
    ...
    logging.info(f"Starting ENHANCED LLM extraction for run_id={run_id} ({len(inputs)} files to process)")
    ...
    for in_key in inputs:
        ...
        try:
            ...
            # Output path: uses 'jsonl_llm_enhanced/' folder
            out_prefix = in_key.rsplit("/", 2)[0] + "/jsonl_llm_enhanced"
            out_key = out_prefix + f"/{post_id}_llm_enhanced.jsonl"
    ...
    result = {
        "ok": True,
        "version": "extractor-llm-enhanced-v1",
        "run_id": run_id,
        "processed": processed,
        "written": written,
        "skipped": skipped,
